    ...
    def run_command(args, stdin_io = nil)
      ...
      Open3.popen3(*args) do |stdin, stdout, stderr, wait_thr|
        ...
        out_thread = Thread.new do
          until stdout.eof?
            stdout_buf << stdout.readpartial(16_384)
          end
        rescue EOFError
          # noop
        end
        ...
        err_thread = Thread.new do
          until stderr.eof?
            stderr_buf << stderr.readpartial(16_384)
          end
        rescue EOFError
          # noop
        end
      ...
      if status && status.exitstatus != 0
        raise ExecutionError.new("tippecanoe failed (exit #{status.exitstatus})",
                                 status: status, stdout: stdout_buf.dup, stderr: stderr_buf.dup)
      end
