...
RSpec.describe('CoreDataConnector::OpenGeographies::V1::Places', type: :request) do
  ...
  it 'supports text search, facet filtering with counts, and pagination' do
    ...
    # count as a 6th place in the very listing this test asserts totals 5 -
    # the "Contained In" relationship works fine across projects for this
    ...
    expect(response_json[:meta][:total_count]).to(eq(5))
    ...
    expect(response).to(have_http_status(:ok))
    expect(response_json[:results].size).to(eq(2))
    expect(response_json[:meta]).to(eq({ page: 2, per_page: 2, total_count: 5, total_pages: 3 }))
  end
end
